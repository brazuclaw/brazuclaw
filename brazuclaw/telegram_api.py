...
def baixar_anexo(token: str, file_id: str) -> tuple[str, bytes]:
    ...
    resposta.raise_for_status()
    return mimetype, resposta.content
...
def enviar_anexo(token: str, chat_id: int, nome: str, anexo_b64: str, mimetype: str, legenda: str = "") -> dict:
    ...
    conteudo = base64.b64decode(anexo_b64.encode("ascii"))
    campo = "photo" if mimetype.startswith("image/") else "document"
    metodo = "sendPhoto" if campo == "photo" else "sendDocument"
    dados = {"chat_id": str(chat_id)}
    if legenda:
        dados["caption"] = legenda[:1024]
    arquivos = {campo: (nome, conteudo, mimetype or "application/octet-stream")}
    resposta = requests.post(_url_api(token, metodo), data=dados, files=arquivos, timeout=80)
    resposta.raise_for_status()
    retorno = resposta.json()
    if not retorno.get("ok"):
        raise RuntimeError(retorno.get("description", "Erro desconhecido do Telegram"))
