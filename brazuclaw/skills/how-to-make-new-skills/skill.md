...
4. **Segurança e Privacidade:** Nenhuma skill deve remover ou modificar arquivos pessoais do usuário (como documentos em `~/Documents`, fotos em `~/Pictures`, etc.) sem autorização expressa e pontual. Automatizações de limpeza devem ser restritas a diretórios de cache ou temporários conhecidos e, mesmo assim, listar o que será removido antes de agir.
5. **Registro:** Após criar ou modificar uma skill, você deve atualizar o arquivo `~/.brazuclaw/skills/skill-list.md`, adicionando o nome da nova skill e um resumo de sua finalidade para que o BrazuClaw conheça o catálogo.
