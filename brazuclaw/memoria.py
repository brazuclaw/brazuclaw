...
def garantir_banco() -> None:
    ...
    with _conexao() as conexao:
        conexao.execute("CREATE TABLE IF NOT EXISTS estado (chave TEXT PRIMARY KEY, valor TEXT NOT NULL DEFAULT '')")
        conexao.execute("CREATE TABLE IF NOT EXISTS mensagens (id INTEGER PRIMARY KEY AUTOINCREMENT, chat_id INTEGER NOT NULL, update_id INTEGER NOT NULL DEFAULT 0, ator TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'respondida', texto TEXT NOT NULL DEFAULT '', anexo_b64 TEXT NOT NULL DEFAULT '', mimetype TEXT NOT NULL DEFAULT '', nome_arquivo TEXT NOT NULL DEFAULT '', criado_em INTEGER NOT NULL)")
        ...
        conexao.execute("CREATE TABLE IF NOT EXISTS crons (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT NOT NULL, prompt TEXT NOT NULL, schedule TEXT NOT NULL, ativo INTEGER NOT NULL DEFAULT 1, chat_callback_id INTEGER NOT NULL DEFAULT 0, callback_quando TEXT NOT NULL DEFAULT 'erro', timeout_segundos INTEGER NOT NULL DEFAULT 120, proximo_em INTEGER NOT NULL DEFAULT 0, ultima_execucao_em INTEGER NOT NULL DEFAULT 0, ultimo_status TEXT NOT NULL DEFAULT '', pid_atual INTEGER NOT NULL DEFAULT 0, abortar INTEGER NOT NULL DEFAULT 0, criado_em INTEGER NOT NULL)")
...
def update_processado(update_id: int) -> bool:
    ...
    garantir_banco()
    with _conexao() as conexao:
        linha = conexao.execute("SELECT 1 FROM mensagens WHERE update_id = ? AND ator = 'agente' AND status = 'respondida' LIMIT 1", (update_id,)).fetchone()
    return bool(linha)
def registrar_interacao(chat_id: int, ator: str, texto: str = "", anexo_b64: str = "", mimetype: str = "", nome_arquivo: str = "", update_id: int = 0, status: str = "respondida") -> None:
    """Salva uma interacao no historico persistente."""
    garantir_banco()
    with _conexao() as conexao:
        conexao.execute("INSERT OR REPLACE INTO mensagens (chat_id, update_id, ator, status, texto, anexo_b64, mimetype, nome_arquivo, criado_em) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (chat_id, update_id, ator[:20], status[:20], texto[: (LIMITE_RESPOSTA if ator == "agente" else LIMITE_TEXTO)], anexo_b64, mimetype[:120], nome_arquivo[:255], int(time.time())))
def montar_contexto(chat_id: int) -> str:
    """Monta o contexto textual das ultimas interacoes do chat."""
    garantir_banco()
    with _conexao() as conexao:
        linhas = conexao.execute("SELECT ator, texto, anexo_b64, mimetype FROM mensagens WHERE chat_id = ? AND status = 'respondida' ORDER BY id DESC LIMIT ?", (chat_id, LIMITE_CONTEXTO)).fetchall()
    linhas.reverse()
    contexto: list[str] = []
...
def criar_cron(nome: str, prompt: str, schedule: str, chat_callback_id: int = 0, callback_quando: str = "erro", timeout_segundos: int = 120, proximo_em: int = 0) -> int:
    ...
    with _conexao() as conexao:
        cursor = conexao.execute("INSERT INTO crons (nome, prompt, schedule, chat_callback_id, callback_quando, timeout_segundos, proximo_em, criado_em) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (nome[:80], prompt.strip(), schedule.strip(), chat_callback_id, callback_quando[:10], max(30, timeout_segundos), proximo_em, int(time.time())))
